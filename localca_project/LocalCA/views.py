    ...
    for root in roots:
        ...
        for intermediate in intermediates:
            ...
            intermediate_list.append({
                ...
                'is_owner': request.user.is_authenticated and intermediate.created_by == request.user,
                'leaves': [
                    {
                        'cert': leaf,
                        'is_owner': request.user.is_authenticated and leaf.created_by == request.user
                    } for leaf in leaves
                ],
            })
        ...
        certificate_tree.append({
            'root': root,
            'is_owner': request.user.is_authenticated and root.created_by == request.user,
            'intermediates': intermediate_list,
        })

    return render(request, "LocalCA/homepage.html", {
...
def create_ca(request):
    ...
    existing_intermediates = IntermediateCertificate.objects.all().order_by('-created_at')
    
    # Get only the user's root certificates for the dropdown
    user_roots = RootCertificate.objects.filter(created_by=request.user).order_by('-created_at')
    ...
    if request.method == "POST":
        ...
        if form_type == "root":
            ...
            try:
                ...
                root_cert_data = ca_manager.create_root_certificate(ca_name, validity_days)
            ...
            if not RootCertificate.objects.filter(id=root_id, created_by=request.user).exists():
                messages.error(request, "You can only sign intermediates with your own root certificates.")
            ...
            try:
                ...
                messages.success(request, f"Intermediate CA '{intermediate_name}' created successfully!")
                return redirect("create_ca")
            
            except Exception as e:
                messages.error(request, f"Error creating intermediate CA: {str(e)}")
    
    return render(request, "LocalCA/create_ca.html", {
...
def create_leaf(request):
    ...
    if request.method == "POST":
        try:
            ...
            messages.success(request, f"Leaf certificate '{common_name}' created successfully!")
    ...
    return render(request, "LocalCA/create_leaf.html", {
...
def revoke_certificate(request, cert_id):
    ...
    try:
        ...
        if RevokedCertificate.objects.filter(certificate=cert).exists():
            messages.warning(request, f"Certificate '{cert.common_name}' is already revoked.")
        else:
            ...
            messages.success(request, f"Leaf certificate '{cert.common_name}' revoked successfully!")
...
def change_password(request):
    ...
    return render(request, 'LocalCA/change_password.html', {
        'form': form
    })
